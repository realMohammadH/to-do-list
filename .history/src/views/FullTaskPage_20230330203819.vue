...
const message = ref("IN");

const emit = defineEmits(["message-sent"]);

const handleButtonClick = () => {
  const message = "Button clicked!";
  // emit the "button-clicked" event with the message as the payload
  emit("button-clicked", message);
};
</script>

<template>
  <wrapper class="overlay" width="100%" height="100%" v-show="showTask">
    <wrapper
      class="full-task-page"
      width="864px"
      height="100%"
      padding="30px 40px"
    >
      <flex class="task-wrapper-info" flow="column" alignItems="end">
        <flex class="header">
          <delete-button></delete-button>
          <close-button @click="handleButtonClick()"></close-button>
        </flex>
