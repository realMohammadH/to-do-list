...
import EditTask from "../../buttons/EditTask.vue";
import FullTaskPage from "../../../views/FullTaskPage.vue";

let taskDescription = ref(true);
function expandTask() {
  taskDescription.value = !taskDescription.value;
}
const showFullTaskPage = ref(false);
function showFullTask() {
  showFullTaskPage.value = !showFullTaskPage.value;
  console.log(showFullTaskPage.value);
}
const buttonClicked = ref(false);
const buttonMessage = ref("");

onMounted(function handleButtonClicked(message) {
...
const count = ref(0);
const dataFromCloseButton = ref(null);
console.log(dataFromCloseButton.value);
...
<template>
  ...
  >
    <input type="checkbox" id="scales" name="scales" @click="expandTask()" />
    <div class="taskInfo">
      <h3 class="task-title">Title</h3>
      <p class="task-description" v-show="taskDescription">
      ...
      <flex class="edit" justifyContent="flex-end">
        <edit-task></edit-task>
      </flex>
    </div>
  </flex>
  <teleport to="body">
    <full-task-page
      v-show="showFullTaskPage"
      @add="(i) => (count += i)"
      @customChange="data = dataFromCloseButton"
    ></full-task-page>
  </teleport>
</template>
